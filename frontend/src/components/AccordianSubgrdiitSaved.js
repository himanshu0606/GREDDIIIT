...
export default function AccordianSubgrdiitSaved(props) {
    ...
    const unlikePost = async (event) => {
        const buttonClicked = event.target;
        const id = buttonClicked.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.id;
        ...
        return (
            <div>
                <div style={{ display: "flex", justifyContent: "center" }}>
                    <div className="card" style={{ width: "80%" }} id={props.id}>
                        <div className="card-header cardHeaderPost">
                            <div className='FollowPost'>
                                <div style={{ display: "flex", marginBottom: "10px" }}>
                                    ...
                                    <div className='postedBy' style={{ marginRight: "20px" }}>
                                        <span>Posted By</span>
                                        <span>{props.postedBy}</span>
                                    </div>
                                    <div className='postedBy'>
                                        <span>SubGreddiit</span>
                                        <span>{props.sub}</span>
                                    </div>
                                </div>
                                <div>
                            ...
                            <div className='LikeDislikeWrapper'>
                                <div className='LikeDislike'>
                                    <span style={{ border: "1px solid #fff", display: "flex", padding: "5px 10px", borderTopLeftRadius: "20px", borderBottomLeftRadius: "20px" }}>
                                        {liked < 1 && <AiOutlineLike size={20} style={{ marginRight: "2px", cursor: "pointer" }} onClick={likePost} />}
                                        {liked > 0 && <AiFillLike size={20} style={{ marginRight: "2px", cursor: "pointer" }} onClick={unlikePost} />}
                                    ...
                                    <span style={{ border: "1px solid #fff", display: "flex", padding: "5px 10px", borderTopRightRadius: "20px", borderBottomRightRadius: "20px" }}>
                                        {disliked < 1 && <AiOutlineDislike size={22} style={{ marginRight: "2px", cursor: "pointer" }} onClick={dislikePost} />}
                                        {disliked > 0 && <AiFillDislike size={20} style={{ marginRight: "2px", cursor: "pointer" }} onClick={dislikePostDelete} />}
                                    </span>
                                </div>
                                <div className='CommentSave'>
                                    ...
                                    <span style={{ marginRight: "5px" }}>
                                        {saved < 1 && <BsBookmark size={25} style={{ marginRight: "5px", cursor: "pointer" }} onClick={savePost} />}
                                        {saved > 0 && <BsBookmarkFill size={25} style={{ marginRight: "5px", cursor: "pointer" }} onClick={unsavePost} />}
                                        <span>Save</span>
                                    </span>
                                    <span style={{ color: "red", fontWeight: "500", cursor: "pointer" }} data-bs-toggle="modal" data-bs-target="#Report" >
